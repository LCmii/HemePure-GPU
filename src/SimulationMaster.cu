...
*/
	...
	// All processes use GPU - including rank 0 for both coordination and computation
        if ( dev_count == 1 )  {
            bool status = deviceAttach(0);
            if (!status) {
                fprintf(stderr, "GPU device setting failed\n");
            ...
            // Round-robin GPU binding for all ranks
            bool status = deviceAttach(localRank % dev_count);
            fprintf(stderr, "LocalRank %d attaching to GPU Device %d\n", localRank, localRank % dev_count);
            if (!status) {
                fprintf(stderr, "GPU device setting failed\n");
                Abort();
            }
